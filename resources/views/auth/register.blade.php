<x-layouts.auth title="Create Account">
    <flux:heading size="xl" class="text-center">Create your account</flux:heading>
    <flux:text class="mt-2 text-center">
        Already have an account? <flux:link href="{{ route('customer.login') }}">Sign in</flux:link>
    </flux:text>

    <form action="{{ route('customer.register') }}" method="POST" class="mt-8 space-y-6">
        @csrf

        <div class="space-y-4">
            <flux:field>
                <flux:label>Name</flux:label>
                <flux:input name="name" type="text" autocomplete="name" required value="{{ old('name') }}" placeholder="Enter your name" :invalid="$errors->has('name')" />
                <flux:error name="name" />
            </flux:field>

            <flux:field>
                <flux:label>Email address</flux:label>
                <flux:input name="email" type="email" autocomplete="email" required value="{{ old('email') }}" placeholder="Enter your email address" :invalid="$errors->has('email')" />
                <flux:error name="email" />
            </flux:field>

            <flux:field>
                <flux:label>Password</flux:label>
                <flux:input name="password" type="password" autocomplete="new-password" required viewable placeholder="Create a password" :invalid="$errors->has('password')" />
                <flux:error name="password" />
            </flux:field>

            <flux:field>
                <flux:label>Confirm Password</flux:label>
                <flux:input name="password_confirmation" type="password" autocomplete="new-password" required viewable placeholder="Confirm your password" />
            </flux:field>
        </div>

        <flux:button type="submit" variant="primary" class="w-full">Create account</flux:button>

        <flux:text class="text-center text-xs">
            By creating an account, you agree to our
            <flux:link href="{{ route('terms-of-service') }}">Terms of Service</flux:link>
            and
            <flux:link href="{{ route('privacy-policy') }}">Privacy Policy</flux:link>.
        </flux:text>
    </form>

    <flux:separator text="Or" class="my-6" />

    <flux:button href="{{ route('login.github') }}" variant="ghost" class="w-full">
        <svg class="size-5" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
            <path fill-rule="evenodd" d="M10 0C4.477 0 0 4.484 0 10.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0110 4.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.203 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.942.359.31.678.921.678 1.856 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0020 10.017C20 4.484 15.522 0 10 0z" clip-rule="evenodd" />
        </svg>
        Sign up with GitHub
    </flux:button>

    <flux:text class="mt-4 text-center text-xs">
        By signing up with GitHub, you agree to our
        <flux:link href="{{ route('terms-of-service') }}">Terms of Service</flux:link>
        and
        <flux:link href="{{ route('privacy-policy') }}">Privacy Policy</flux:link>.
    </flux:text>
</x-layouts.auth>
