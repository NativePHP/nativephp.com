            ...
            <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
                <div class="flex items-start">
                    <div class="flex-shrink-0">
                        <svg class="h-6 w-6 text-purple-700 dark:text-purple-300" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <div class="ml-4">
                        ...
                        <div class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                            @if($user->github_username)
                                <p>Connected as <span class="font-mono font-medium text-gray-900 dark:text-white">{{ '@' . $user->github_username }}</span></p>
                                ...
                                @endif
                            @else
                                <p>Connect your GitHub account to access the Plugin Dev Kit repository.</p>
                            @endif
                        </div>
                    </div>
                </div>
                <div class="flex flex-wrap gap-2 lg:flex-shrink-0">
                    @if($user->github_username)
                        ...
                        @elseif($collaboratorStatus === 'pending')
                            <button wire:click="refreshStatus" type="button" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-purple-600 hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-purple-500">
                        ...
                        @else
                            <form action="{{ route('github.request-claude-plugins-access') }}" method="POST">
                                @csrf
                        ...
                        @endif
                    @else
                        <a href="{{ route('github.redirect', ['return' => route('customer.integrations')]) }}" class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-white bg-gray-900 dark:bg-white dark:text-gray-900 hover:bg-gray-800 dark:hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                            Connect GitHub
                        </a>
                    @endif
                </div>
