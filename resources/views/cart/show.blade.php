<x-layout title="Your Cart">
    <div class="mx-auto mt-10 max-w-4xl px-4 sm:px-6 lg:px-8">
        {{-- Header --}}
        <header class="mb-8">
            <a href="{{ route('plugins.marketplace') }}" class="inline-flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300">
        ...
        @if ($cart->isEmpty())
            ...
            <div class="rounded-lg border border-gray-200 bg-white p-12 text-center dark:border-gray-700 dark:bg-gray-800">
                ...
                <p class="mt-2 text-sm text-gray-500 dark:text-gray-400">Browse our plugin marketplace to find plugins for your app.</p>
                <a href="{{ route('plugins.marketplace') }}" class="mt-6 inline-flex items-center rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700">
                    Browse Plugins
                </a>
            </div>
        @else
                                ...
                                @else
                                    ...
                                    >
                                        ...
                                        <div class="flex flex-1 flex-col">
                                            ...
                                            <div class="mt-4 flex justify-end">
                                                <form action="{{ route('cart.remove', $item->plugin->routeParams()) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="text-sm font-medium text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300">
                                                        Remove
        ...
        @if ($bundleUpgrades->isNotEmpty())
            <div class="mt-8">
                ...
                <div class="relative">
                    <div class="flex gap-4 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-600">
                        @foreach ($bundleUpgrades as $bundle)
                            <div class="w-72 shrink-0 snap-start rounded-lg border-2 border-amber-400 bg-gradient-to-br from-amber-50 to-orange-50 p-4 dark:border-amber-500 dark:from-amber-950/30 dark:to-orange-950/30">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        <h3 class="truncate text-sm font-semibold text-amber-900 dark:text-amber-200">
                                            {{ $bundle->name }}
                                        </h3>
                    ...
                    @if ($bundleUpgrades->count() > 1)
                        <div class="pointer-events-none absolute inset-y-0 right-0 w-12 bg-gradient-to-l from-white dark:from-gray-900"></div>
                    @endif
                </div>
            </div>
        @endif
